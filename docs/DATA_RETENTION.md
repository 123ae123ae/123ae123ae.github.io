...
版本：2026-07-17
适用范围：Ham Ham 生产环境（Supabase 项目 `vqxzrydqnlpxyjafjdoh`）、App 设备离线副本和迁移快照。
...
| 隐私政策阅读确认记录 | 账户有效期间 | 删除账户时通过外键级联删除；如发生争议需另行法律保全，必须记录原因和期限 |
| 设备离线副本和待同步队列 | 直到同步/覆盖、删除账户或清除 App 数据 | 账户删除流程清理本机数据；用户也可在系统设置中清除 App 数据 |
